...
from lovktv.contracts import RoomAction


def normalize_room_code(value: Any) -> str:
    code = str(value or "").strip().upper()
    if not code or len(code) > 32 or any(not (char.isalnum() or char in "-_") for char in code):
...
def normalize_command(action: Any, payload: Mapping[str, Any] | None = None) -> dict[str, Any]:
