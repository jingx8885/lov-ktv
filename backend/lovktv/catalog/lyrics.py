...
from .search import BROWSER_UA, TONZHON_API, post_form

LRC_LINE = re.compile(r"^\[(\d+):(\d+(?:\.\d+)?)\](.*)$")
META_PREFIX = ("作词", "作曲", "编曲", "作詞", "編曲", "制作人", "製作人", "制作", "Lyrics", "lyrics by", "composer", "arranger", "Producer", "producer")
...
def parse_lrc(lrc: str) -> list[dict[str, Any]]:
    ...
    for item in out:
        if item.get("end_ms") is not None and int(item["end_ms"]) <= item["ms"]:
            item = dict(item); item.pop("end_ms", None)
