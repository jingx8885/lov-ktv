...
def _complete_mugen_audio(skeleton: dict[str, Any], out_dir: Path, query: str) -> dict[str, Any]:
    """Keep official Mugen lyrics; if kara.moe media is unreachable, pull audio elsewhere."""
    mp3_path = out_dir / "original.mp3"
    if mp3_path.exists() and mp3_path.stat().st_size > 200:
        return skeleton
    title = str(skeleton.get("title") or query)
    artist = str(skeleton.get("artist") or "")
    ...
    filled = ""
    hit = pick_bilibili_mv(short, artist) or pick_bilibili_mv(clean_search_title(short), "")
    ...
    if not filled:
        ok, got_title = try_ytdlp_search(f"{short} {artist}".strip() or query, mp3_path, "soundcloud")
        if ok:
            filled = "soundcloud"
            audio["title"] = got_title
    if not filled:
        ok, got_title = try_ytdlp_search(f"{short} {artist}".strip() or query, mp3_path, "youtube")
        if ok:
            filled = "youtube"
            audio["title"] = got_title
    ...
    audio["source"] = f"mugen-{filled}"
    audio["needs_separate"] = True
    skeleton["audio"] = audio
    skeleton["source"] = source
    skeleton["needs_separate"] = True
...
def import_song(
    *,
    ...
    if song_id and is_mugen_kid(song_id):
        skeleton = import_mugen_song(song_id, out_dir, query=query)
        return _complete_mugen_audio(skeleton, out_dir, query)
    if not song_id:
        chosen = pick_vocal_hit(search_mugen(query, count=8).get("hits") or [])
        if chosen and chosen.get("id"):
            skeleton = import_mugen_song(str(chosen["id"]), out_dir, query=query)
            return _complete_mugen_audio(skeleton, out_dir, query)
