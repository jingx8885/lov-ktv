...
from lovktv.catalog import audio as audio_provider
from lovktv.catalog import bilibili
from lovktv.catalog import kugou as kugou_provider
from lovktv.catalog import lyrics as lyrics_provider
from lovktv.catalog import mugen as mugen_provider

from . import search as search_provider
...
def _complete_mugen_audio(
    ...
    filled = ""
    hit = audio_provider.pick_bilibili_mv(short, artist) or audio_provider.pick_bilibili_mv(
        search_provider.clean_search_title(short), ""
    )
    if hit and audio_provider.try_bilibili_download(str(hit["bvid"]), mp3_path, mtv_path):
        filled = "bilibili"
    ...
    if not filled:
        ok, got_title = audio_provider.try_ytdlp_search(
...
def import_song(
    *, query: str, out_dir: Path, song_id: str | None = None, prefer_ytdlp: bool = False
    ...
    if song_id and mugen_provider.is_mugen_kid(song_id):
        return _complete_mugen_audio(
            mugen_provider.import_mugen_song(song_id, out_dir, query=query), out_dir, query
        )
    if not song_id:
        chosen = mugen_provider.pick_vocal_hit(mugen_provider.search_mugen(query, count=8).get("hits") or [])
        if chosen and chosen.get("id"):
            return _complete_mugen_audio(
                mugen_provider.import_mugen_song(str(chosen["id"]), out_dir, query=query),
                out_dir,
                query,
            )
    results = search_provider.search_tonzhon(query)
    ...
    if song_id:
        ...
        chosen = results[0]
        for item in results:
            if search_provider.is_clean_title(str(item.get("name") or "")):
                chosen = item
                break
    title_name, artist_name = (
        str(chosen.get("name") or query),
        search_provider.flatten_artists(chosen),
    )
    kugou = kugou_provider.fetch_kugou_lyrics(title_name, artist_name)
    ...
    if kugou and kugou.get("timeline"):
        ...
        lrc = lyrics_provider.fetch_lyric(lyric_id) if lyric_id.isdigit() else ""
        if not lrc.strip():
            for song in search_provider._tonzhon_hits(title_name, 5, 1):
                sid = str(song.get("id") or "")
                if sid.isdigit():
                    try:
                        lrc = lyrics_provider.fetch_lyric(sid)
                    except Exception:
                        lrc = ""
                    if lrc.strip():
                        break
        ...
        lines = lyrics_provider.parse_lrc(lrc)
    if not lines:
        raise RuntimeError("歌词为空")
    audio_file = None
    audio_source = "none"
    ...
    cached = audio_provider.peek_audio_source(chosen_id)
    pinned_bvid = str(
        cached.get("bvid")
        or (chosen_id if bilibili.is_bvid(chosen_id) else "")
        or (song_id if bilibili.is_bvid(song_id or "") else "")
    )
    if pinned_bvid and audio_provider.try_bilibili_download(pinned_bvid, mp3_path, mtv_path):
    ...
    if (
        audio_file is None
        and cached.get("page")
        and audio_provider._ytdlp_download(str(cached["page"]), mp3_path)
    ):
    ...
    ytdlp_query = (
        f"{chosen.get('name') or ''} {search_provider.flatten_artists(chosen)}".strip() or query
    )
    if audio_file is None:
        hit = audio_provider.pick_bilibili_mv(title_name, artist_name) or audio_provider.pick_bilibili_mv(
            search_provider.clean_search_title(title_name), ""
        )
        if hit and audio_provider.try_bilibili_download(str(hit["bvid"]), mp3_path, mtv_path):
            audio_file, audio_source, audio_title, audio_bvid, has_video = (
                "original.mp3",
    ...
    if audio_file is None:
        ok, got_title = audio_provider.try_ytdlp_search(ytdlp_query, mp3_path, "soundcloud")
        if ok:
            audio_file, audio_source, audio_title = (
                "original.mp3",
                "soundcloud",
    ...
    if (
        ...
        and audio_provider.try_netease_download(chosen_id, mp3_path)
    ):
        audio_file, audio_source = "original.mp3", "netease"
    if audio_file is None:
        ok, got_title = audio_provider.try_ytdlp_search(ytdlp_query, mp3_path, "youtube")
        if ok:
    ...
    if pic.startswith("http"):
        try:
            ...
            cover_file = ""
    skeleton = {
        "title": f"{chosen.get('name')} · {search_provider.flatten_artists(chosen)}",
        "artist": artist_name,
        "language": language,
