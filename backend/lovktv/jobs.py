...
class JobQueue:
    ...
    def stop(self, timeout: float = 5.0) -> bool:
        """Stop the worker, dropping jobs that have not started yet.

        A running job is allowed to finish for ``timeout`` seconds.  Queued
        jobs are discarded after the worker exits so shutdown never leaves the
        queue's join counter or duplicate-suppression set stuck.
        """
        with self._lock:
    ...
    def health(self) -> dict[str, Any]:
        """Return non-secret worker state for readiness/diagnostics endpoints."""
        with self._lock:
            thread = self._worker_thread
