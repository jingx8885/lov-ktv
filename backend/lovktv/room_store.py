...
from lovktv.db import connect as db_connect, execute
...
class RoomRepository(Protocol):
    ...
    def set_mix(self, code: str, vocal_mix: float | None = None, volume: int | None = None,
                mic_gain: int | None = None, lyric_mode: str | None = None,
                paused: bool | None = None) -> dict[str, Any]: ...
    def set_room_lan(self, code: str, origin: str, mic_port: int | None = None,
                     mic_sample_rate: int | None = None) -> dict[str, Any]: ...
    ...
    with _LOCK, connect() as conn:
        for key in keys:
            ...
            if row:
                ...
                execute(conn, "INSERT INTO hosts (key, room, ua, created_at, last_seen) VALUES (?,?,?,?,?)",
...
def _private_ipv4(host: str) -> bool:
    ...
    try:
        ...
        return False
    return all(0 <= n <= 255 for n in nums) and (nums[0] == 10 or
        (nums[0] == 192 and nums[1] == 168) or (nums[0] == 172 and 16 <= nums[1] <= 31))
...
def room_snapshot(code: str) -> dict[str, Any]:
    ...
    with connect() as conn:
        rows = execute(conn, """SELECT queue.id, queue.song_id, queue.position,
            songs.title, songs.artist, songs.status, songs.language
            FROM queue JOIN songs ON songs.id=queue.song_id
            WHERE queue.room=? ORDER BY queue.position ASC, queue.created_at ASC""", (code,)).fetchall()
    ...
    if items and index < 0:
        ...
        index = 0
        room["now_index"] = 0
    return {**room, "queue": items, "now_playing": items[index] if items and 0 <= index < len(items) else None}
...
def enqueue(code: str, song_id: str) -> dict[str, Any]:
    ...
    with _LOCK, connect() as conn:
        ...
        execute(conn, "INSERT INTO queue (id,room,song_id,position,created_at) VALUES (?,?,?,?,?)", (new_id(), code, song_id, pos, now_ms()))
...
def bump(code: str, item_id: str) -> dict[str, Any]:
    ...
    if idx is None or idx <= snap["now_index"] + 1:
        return snap
    target = items[snap["now_index"] + 1]["position"] if len(items) > snap["now_index"] + 1 else items[idx]["position"]
    with _LOCK, connect() as conn:
...
def set_mix(code: str, vocal_mix: float | None = None, volume: int | None = None,
            mic_gain: int | None = None, lyric_mode: str | None = None,
            paused: bool | None = None) -> dict[str, Any]:
    from lovktv.store import normalize_lyric_mode
    fields = {}
    if vocal_mix is not None: fields["vocal_mix"] = max(0.0, min(1.0, float(vocal_mix)))
    if volume is not None: fields["volume"] = max(0, min(100, int(volume)))
    if mic_gain is not None: fields["mic_gain"] = max(0, min(100, int(mic_gain)))
    if lyric_mode is not None: fields["lyric_mode"] = normalize_lyric_mode(lyric_mode)
    if paused is not None: fields["paused"] = 1 if paused else 0
    allowed = {key: value for key, value in fields.items() if key in ROOM_FIELDS}
    if allowed:
        assignments = ", ".join(f"{key}=?" for key in allowed)
        with _LOCK, connect() as conn:
            execute(conn, f"UPDATE rooms SET {assignments} WHERE code=?", (*allowed.values(), code.upper()))
    return room_snapshot(code)

def set_room_lan(code: str, origin: str, mic_port: int | None = None,
                 mic_sample_rate: int | None = None) -> dict[str, Any]:
    room = str(code or "").strip().upper()
    lan = normalize_lan_origin(origin)
    port = int(mic_port or 0)
    if port < 0 or port > 65535: port = 0
    rate = int(mic_sample_rate or 48000)
    if rate < 8000 or rate > 96000: rate = 48000
    ensure_room(room)
    with _LOCK, connect() as conn:
        execute(conn, "UPDATE rooms SET lan_origin=?,lan_mic_port=?,lan_mic_sample_rate=?,lan_seen_at=? WHERE code=?", (lan, port, rate, now_ms(), room))
