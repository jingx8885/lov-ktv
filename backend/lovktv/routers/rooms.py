...
from lovktv.i18n import localize_exc, localize_error_text, request_lang, translate, ws_lang
...
def api_my_room(request: Request) -> dict:
    code = room_for_hosts(host_keys(host_machine(request), request.headers.get("user-agent") or "", request_ip(request)))
...
def api_create_room(request: Request) -> JSONResponse:
    ...
    return set_host_cookie(JSONResponse(room_view(room["code"], lang=request_lang(request))), request, token)
...
def api_room(request: Request, code: str) -> JSONResponse:
    ...
    return set_host_cookie(JSONResponse(view), request, bind_host(request, view["code"]))
...
def api_room_lan(request: Request, code: str, payload: RoomLanPayload) -> JSONResponse:
    ...
    return set_host_cookie(JSONResponse(room_view(code, snap, lang=request_lang(request))), request, bind_host(request, snap["code"]))
...
def api_bump(code: str, payload: RoomCommandPayload) -> dict:
    return run_command(code.upper(), RoomCommand.from_payload("bump", payload.as_dict()))


@router.post("/api/rooms/{code}/skip")
async def api_skip(request: Request, code: str) -> dict:
    code = code.upper()
    view = room_view(code, run_command(code, RoomCommand.from_payload("skip")), lang=request_lang(request))
    await broadcast(code, {"type": "snapshot", "room": view})
    return view
...
async def ws_room(ws: WebSocket, code: str) -> None:
    ...
    try:
        while True:
            ...
            if action == "hello":
                _peers[ws] = {"code": code, "peer": str(msg.get("peer") or ""), "role": str(msg.get("role") or "")}
                await broadcast(code, {"type": "peer", "event": "join", "peer": _peers[ws]["peer"], "role": _peers[ws]["role"]}, skip=ws)
                continue
            if action == "rtc":
                kind = str(msg.get("kind") or "")
                peer = str(msg.get("from") or _peers.get(ws, {}).get("peer") or "")
                if kind == "offer" and peer: _mics[code] = peer
                if kind == "hangup" and _mics.get(code) == peer: _mics.pop(code, None)
                await broadcast(code, {"type": "rtc", **{k: v for k, v in msg.items() if k != "action"}}, skip=ws)
                if kind in {"offer", "hangup"}: await broadcast(code, {"type": "snapshot", "room": room_view(code, lang=lang)})
                continue
            if action == "mic":
                peer = str(msg.get("from") or _peers.get(ws, {}).get("peer") or "")
                if msg.get("on") and peer: _mics[code] = peer
                elif _mics.get(code) == peer or not peer: _mics.pop(code, None)
                if msg.get("mic_gain") is not None: run_command(code, RoomCommand.from_payload("mix", {"mic_gain": msg.get("mic_gain")}))
                await broadcast(code, {"type": "snapshot", "room": room_view(code, lang=lang)})
                continue
            try:
                if action in {"skip", "bump", "play"}:
                    event = normalize_playback_event(action, msg)
                    snap = run_command(code, RoomCommand.from_payload(event["action"], event))
                elif action == "enqueue": snap = run_command(code, RoomCommand.from_payload("enqueue", msg))
                elif action == "mix": snap = run_command(code, RoomCommand.from_payload("mix", msg))
                else:
                    await ws.send_json({"type": "error", "message": translate(lang, "api.unknown_command")}); continue
            except ValueError as exc:
                await ws.send_json({"type": "error", "message": localize_error_text(lang, str(exc))}); continue
            await broadcast(code, {"type": "snapshot", "room": room_view(code, snap, lang=lang)})
        ...
        if peer and _mics.get(code) == peer:
            ...
            await broadcast(code, {"type": "snapshot", "room": room_view(code, lang=lang)})
        await broadcast(code, {"type": "peer", "event": "leave", "peer": peer, "role": info.get("role") or ""})
