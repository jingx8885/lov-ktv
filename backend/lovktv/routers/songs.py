...
from lovktv.i18n import localize_exc, localize_song, request_lang, t as i18n_t
...
from lovktv.store import (create_song, delete_song, get_song, list_songs, retry_query, update_song,
                          with_media_flags)
...
def api_import(request: Request, payload: dict) -> dict:
    ...
    song = create_song(title=str(payload.get("title") or query), artist=str(payload.get("artist") or ""), language=language, netease_id=raw_id)
    spawn(process_import, song["id"], query, raw_id, language)
    return song


@router.post("/api/songs")
async def api_upload(file: UploadFile = File(...), title: str = Form(""), artist: str = Form(""), language: str = Form("zh"), lyrics: str = Form(""), request: Request = None) -> dict:
...
def api_ja_lyrics(request: Request, payload: dict = Body(default={})) -> dict:
    ...
    try:
        return annotate_ja_lines(lines, title=str(payload.get("title") or ""), artist=str(payload.get("artist") or ""))
    except Exception as exc:
...
def api_realign(request: Request, song_id: str, payload: dict = Body(default={})) -> dict:
    ...
    spawn(process_realign, song_id, payload.get("language") or song.get("language"), bool(payload.get("rebuild_mtv")))
...
def api_retry_song(request: Request, song_id: str) -> dict:
    ...
    spawn(process_import, song_id, retry_query(song), str(song.get("netease_id") or ""), song.get("language"))
    return {"ok": True, "song_id": song_id, "status": "queued"}


@router.get("/api/songs")
def api_songs(request: Request, q: str = "", by: str = "all", letter: str = "", page: int | None = None, count: int = 12, after: str = "") -> dict:
    lang = request_lang(request)
    songs = prefer_native_library([localize_song(lang, with_media_flags(song) or song) for song in list_songs()])
