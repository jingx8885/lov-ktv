...
async def api_upload(
    ...
    dest = store.MEDIA_DIR / song["id"] / "original.mp3"
    with dest.open("wb") as handle:
        shutil.copyfileobj(file.file, handle)
    if lyrics.strip():
        (store.MEDIA_DIR / song["id"] / "lyrics.lrc").write_text(lyrics, encoding="utf-8")
...
def api_save_lyrics(
    ...
    write_subtitles(timeline, store.MEDIA_DIR / song_id)
    write_manual_lrc(store.MEDIA_DIR / song_id, timeline["cues"])
...
def api_song(request: Request, song_id: str) -> dict:
    ...
    if not song:
        fail(request, 404, "api.song_not_found")
    folder = store.MEDIA_DIR / song_id
    song["files"] = (
...
def api_learn(request: Request, song_id: str) -> dict:
    ...
    if not song:
        fail(request, 404, "api.song_not_found")
    path = store.MEDIA_DIR / song_id / "lyrics.json"
    if not path.exists():
