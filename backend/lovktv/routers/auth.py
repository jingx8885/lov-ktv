...
from lovktv.auth import (SESSION_COOKIE, auth_status, decode_state, done_login_path,
    encode_state, exchange_wechat_code, in_wechat, login_page_url, scan_login_url,
    wechat_authorize_url, wechat_ready)
from lovktv.i18n import localize_exc
from lovktv.store import (confirm_login_ticket, consume_confirmed_ticket, create_login_ticket,
    create_session, delete_session, get_login_ticket, upsert_device_user, upsert_wechat_user)
from lovktv.services.http import clear_session, current_user, fail, request_base, set_session
...
def api_auth_scan(request: Request, ticket: str = "", room: str = "", next: str = "") -> RedirectResponse:
    ...
    if user:
        if ticket:
            try:
                ...
                pass
        return RedirectResponse(dest if dest.startswith("http") else f"{base}{dest}", status_code=302)
...
def api_wechat_login(request: Request, silent: bool = False, quick: bool = False, ticket: str = "", next: str = "") -> RedirectResponse:
...
def api_auth_qr(request: Request, payload: dict = Body(default={})) -> dict:
    ...
    return {**ticket, "url": scan_login_url(request_base(request), ticket=ticket["ticket"], room=room)}
