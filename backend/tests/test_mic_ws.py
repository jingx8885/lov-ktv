...
def test_mix_http_sets_mac_volume_and_snapshot(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(room_runtime, "set_host_volume", lambda volume: applied.append(volume) or True)
    monkeypatch.setattr(room_runtime, "host_volume_meta", lambda: {"host_volume_kind": "mac", "host_volume": 35})
