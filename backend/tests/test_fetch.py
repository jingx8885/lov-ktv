from lovktv.catalog import audio, importer, mugen, search, lyrics, kugou
...
def _no_mugen(monkeypatch):
    ...
    monkeypatch.setattr(mugen, "is_mugen_kid", lambda value: False)
    monkeypatch.setattr(kugou, "fetch_kugou_lyrics", lambda *args, **kwargs: None)
    monkeypatch.setattr(audio, "pick_bilibili_mv", lambda *args, **kwargs: None)
    monkeypatch.setattr(audio, "try_bilibili_download", lambda *args, **kwargs: False)
...
def test_import_uses_pinned_netease_id(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lyrics, "fetch_lyric", lambda song_id, source="netease": "[00:01.00]Give a reason")
    downloaded: list[str] = []
    ...
    monkeypatch.setattr(audio, "try_netease_download", fake_download)
    monkeypatch.setattr(audio, "try_ytdlp_search", lambda *args, **kwargs: (False, "should not run"))
...
def test_import_uses_previewed_ytdlp_page(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lyrics, "fetch_lyric", lambda song_id, source="netease": "[00:01.00]Give a reason")
    monkeypatch.setattr(audio, "try_netease_download", lambda song_id, path: False)
    downloaded: list[str] = []
    monkeypatch.setattr(audio, "_ytdlp_download", lambda page, path: downloaded.append(page) or path.write_bytes(b"x" * 60_000) or True)
