...
from lovktv.catalog import audio, netease
...
def test_eapi_params_roundtrip_with_openssl():
    if not shutil.which("openssl"):
        ...
        pytest.skip("openssl is required for eapi interoperability check")
...
def test_probe_uses_eapi_url(monkeypatch):
    ...
    monkeypatch.setattr(audio, "eapi_play_url", lambda song_id: "http://m8.music.126.net/x.mp3")
    assert audio.probe_netease_url("33894312") is True
    monkeypatch.setattr(audio, "eapi_play_url", lambda song_id: "")
    assert audio.probe_netease_url("186016") is False
...
def test_download_uses_eapi_cdn_and_proxy(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(audio, "eapi_play_url", lambda song_id: "http://m8.music.126.net/ok.mp3")
    seen = {}
    ...
    assert audio.try_netease_download("33894312", tmp_path / "a.mp3") is True
