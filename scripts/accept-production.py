#!/usr/bin/env python3
"""Smoke-test the public KTV deployment without credentials."""
...
def fetch(base: str, path: str) -> tuple[int, bytes]:
    ...
    try:
        ...
        print(f"{path}: request failed ({exc.reason})", file=sys.stderr)
        return 0, b""
    ...
    for path in PATHS:
        ...
        if path == "/api/host" and ok:
            ...
            if ok and args.expect_origin and host.get("origin") != args.expect_origin:
                print(f"{path}: unexpected origin", file=sys.stderr)
                ok = False
