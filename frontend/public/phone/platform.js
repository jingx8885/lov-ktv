...
function privateHttpHost(url) {
  try {
    ...
    return (nums[0] === 192 && nums[1] === 168) || nums[0] === 10 ||
...
export function nativeCapabilities() {
  ...
  try { return JSON.parse(native.capabilities() || "{}"); } catch (_) {
    return { native: false, tv: false, iem: false, host: "", port: 0, rate: 0 };
  }
}
...
export function nativeMicState() {
  ...
  try { return JSON.parse(native.state() || "{}"); } catch (_) {
...
function waitMicPermission() {
  return new Promise((resolve, reject) => {
    ...
    const finish = (ok) => {
      ...
      clearTimeout(timer);
      if (ok) resolve(); else reject(new Error(t("phone.mic.allow")));
    };
...
export function setNativeGain(value) {
  ...
  try { native.setGain(Math.max(0, Math.min(100, Number(value) || 0))); } catch (_) {}
}
...
export function scanTv() {
  ...
  try { native.scanTv(); return true; } catch (_) { return false; }
}

export function useLan(lan, room) {
  const native = bridge();
  if (!native || typeof native.useLan !== "function") return false;
  try { native.useLan(String(lan || ""), String(room || "")); return true; } catch (_) { return false; }
}
...
function nativeFetchJson(url, opts) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => { delete pendingHttp[id]; reject(new Error("lan-timeout")); }, 20000);
    pendingHttp[id] = (msg) => {
      clearTimeout(timer); delete pendingHttp[id];
      let data = {};
      try { data = typeof msg.body === "string" ? JSON.parse(msg.body || "{}") : (msg.body || {}); } catch (_) {}
      resolve({ ok: !!msg.ok, status: Number(msg.status) || 0, data });
    };
    try { native.http(id, url, method, body); } catch (err) {
      clearTimeout(timer); delete pendingHttp[id]; reject(err);
    }
  });
}

export const phonePlatform = {
  mic: { hasNative: hasNativeMic, capabilities: nativeCapabilities, state: nativeMicState, call: nativeCall, setGain: setNativeGain },
  scanner: { available: hasNativeScan, scan: scanTv, useLan },
  media: { url(path) { return String(path || ""); } },
  remote: { open(url) { try { return !!window.open(url, "lovktv-tv"); } catch (_) { return false; } } },
  http: {
...
export function webMicApi() {
  try { return typeof window !== "undefined" ? window.LovMic || null : null; } catch (_) { return null; }
}

export function micErrorText(err) {
  const api = webMicApi();
  try { return (api && typeof api.micErrorText === "function" && api.micErrorText(err)) || (err && err.message) || t("phone.mic.fail"); } catch (_) {
